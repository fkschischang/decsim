...
/*
	mc-decsim-hosc is a simulator for multiply-chained higher-order staircase
	codes with extended Hamming component codes and an arbitrary number C of
	chain links. This simulator requires C > 1. If C = 1 is required, decsim-hosc
	can be used instead.

	mc-decsim-hosc is accompanied by a pedagogical Julia model available
	at https://github.com/applecoffeecake/mc-hosc-Julia
*/

/*
	Key parameters:

	L:	Number of columns of square blocks from which to form a rectangle (number of "classes").
	T:	Sidelength of a square block; each square block contains T*T bits.
	M:	Memory/bit-degree parameter. Extended Hamming components have length L*T*(M+1).
	C:	Number of rows of square blocks from which to form a rectangle (number "chain links").
	W:	Decoding window size in number of rectangles.
	F:	Frame length. Number of rectangles from which to form a frame.

	BatchSize: number of frames to process between error-count valuations.
...
/*
	...
	An unterminated codeword is a stream of (C*T)-by-(L*T) rectangles of which
	...
	Specify a frame length F in numbers of rectangles to simulate the block
	...
	(C*T)-by-(L*T-parity) information portion so that only the (C*T)-by-parity
	parity portion needs to be transmitted.
	As a result, the total number of bits transmitted is
	(F-W)*(C*T)*(L*T) + W*(C*T)*parity of which (F-W)*(C*T)*(L*T-parity) is
	information. The ratio of these two quantities is the overall rate of the
	block code.
...
// Receiver circular buffer of W rectangles of CTLT bits.
// CT x LT
// Within a rectangle are C rows of L blocks (squares)
// of TT bits with row-major order for bits within a block.
// So, we run through rows within a block, blocks within a row,
// then rows within a rectangle.
unsigned char RXbuffer[W*CTLT];
...
/*
 * Circular buffer of W groups of CT syndromes.
 * The group with modulo-W index i consists of the CT syndromes
 * corresponding to the CT component codewords which START in rectangle i.
 ...
 * 	https://github.com/applecoffeecake/mc-hosc-Julia
...
static inline __attribute__((always_inline)) void flip(uint32_t rect, uint32_t c_link, uint32_t c_class, uint32_t posn, uint32_t age) {
    int i, j, k;
    unsigned char *p = RXbuffer + rect*CTLT + c_link*TLT + c_class*TT + posn;
    if (*p) { /* is this a one? */
        --RectWeight[rect];
        *p = '\0';
    } else {
    ...
    while (1) {
        ...
        j = col(posn,k);
		if (k == 0) { // link
			Syndrome[((rect-SCOPE_MINUS_DTS_c[c_class][k]+W)%W)*CT + c_link*T + i] ^= synFromErrorloc(((M-k)*L+c_class)*T+j);
		} else { // link+1 mod C
			Syndrome[((rect-SCOPE_MINUS_DTS_c[c_class][k]+W)%W)*CT + (c_link == 0 ? C-1 : c_link-1)*T + i] ^= synFromErrorloc(((M-k)*L+c_class)*T+j);
		}
        k--;
    }
}
...
static inline __attribute__((always_inline)) void ReceiveRect(float p) {
    ...
    memset(RXbuffer+NewestRect*CTLT, 0, CTLT*sizeof(unsigned char));
    memset(Syndrome+NewestRect*CT, 0, CT*sizeof(uint32_t));
    ...
    while (1) {
        ...
        if (rectposn >= CTLT) {
			...
			flip(NewestRect, rectposn/TLT, (rectposn%TLT)/TT, (rectposn%TLT)%TT, 0);
		}
    }
}
...
static inline __attribute__((always_inline)) void ReceivePseudoterminationRect(float p) {
    ...
    while (1) {
        ...
        if (rectposn >= CTLT) {
			break;
		} else if (((rectposn%TLT)/TT)*T + ((rectposn%TLT)%TT)%T >= (LT-parity)) {
			flip(NewestRect, rectposn/TLT, (rectposn%TLT)/TT, (rectposn%TLT)%TT, 0);
		}
    }
}
...
static inline __attribute__((always_inline)) void Initialize() {
    /* zero global arrays */
    memset(RXbuffer, 0, W*CTLT*sizeof(unsigned char));
    memset(Syndrome, 0, W*CT*sizeof(uint32_t));
...
int sweep() {
	...
	int j_rect, c_class, c_link; // error column within rectangle, class index, link index
	int k;      /* loop index                    */
	...
	for (k=0; k < W-SCOPE; ++k) {
		for (c_link=0; c_link < C; ++c_link) {
			for (i=0; i < T; ++i) {
				syn = Syndrome[rect*CT + c_link*T + i];
				if (syn & 0x01) {
					...
					if (errorloc < LT*(M+1)) {
						...
						c_class = j_rect/T;
						if (perm == 0) {
							flip((rect+SCOPE_MINUS_DTS_c[c_class][perm])%W,
									c_link, c_class, pos(i, j, perm),
									k+SCOPE-SCOPE_MINUS_DTS_c[c_class][perm]);
						} else {
							flip((rect+SCOPE_MINUS_DTS_c[c_class][perm])%W,
									(c_link == C-1 ? 0 : c_link+1), c_class, pos(i, j, perm),
									k+SCOPE-SCOPE_MINUS_DTS_c[c_class][perm]);
						}
						count += 1;
					}
...
void SanityCheck()  /* triggered by -C command line switch */
{
    ...
    printf("L=%d, T=%d, M=%d, C=%d, Hamming Code Length=%d, Parity bits=%d, Rate=%f\n",
           L, T, M, C, LT*(M+1), parity, R);
    Initialize();
    ...
    for (i=-2; i <= 2; ++i)
    {
        ...
        qq = (float)ErrorCount/SanityCheckRectangles/(CTLT);
...
int main(int argc, char **argv){
	...
	if (C == 1) {
		printf("C = 1 not supported! Use decsim-hosc instead.\n");
		return 1;
	}
    ...
    if (VERBOSE)
    {
       /* print some useful information */
        fprintf(stderr,
            "Simulating L = %d, M = %d, SCOPE = %d, T = %d, C = %d, (M+1)LT = %d, %d parity bits,\nR = %lf",
						L, M, SCOPE, T, C, (M+1)*LT, parity, R);
        fprintf(stderr,
		...
		for (frame=0; frame < ActualBatchSize; ++frame) {			
			...
			if (VERBOSE && !(FramesDecoded & STATUS_FREQUENCY)) {
				...
				"%d bit errors, %d frame errors in %lu frames delivered; %g bits/s = %g frames/s					\r", 
						...
						(double)CT*(LT-parity)*(F-W)*FramesDecoded/(double)(now-then),
	...
	printf("\nFINAL RESULTS:\n");
	printf("Parameters:\n");
	printf("Simulated L = %d, M = %d, SCOPE = %d, T = %d, C = %d, (M+1)LT = %d, %d parity bits,\nR = %lf",
            L, M, SCOPE, T, C, (M+1)*LT, parity, R);
	...
	printf(" Total of %lu = %g information bits delivered.\n", 
							(F-W)*CT*(LT-parity)*FramesDecoded,
					(double)(F-W)*CT*(LT-parity)*FramesDecoded);
    printf(" BER = %g\n", (double)BitErrors/(F-W)/(CT*(LT-parity))/FramesDecoded);
